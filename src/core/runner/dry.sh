...
___ () { #HELP <yaml_file>|Display the runner bash script without executing it
  ...
  do
    ...
    source /proc/self/fd/0 <<< "$(json::program --slurpfile JSON <(print '%s' "${json}") --slurpfile IMPORT <(print '{"import": %s}' "${import:-"{}"}") --arg ROOT "$(dirname "${filepath}")/" "${jq[yml2bash/common]}"'
      (if ($JSON | type == "array") then $JSON[0] else $JSON end) as $JSON |
      ...
      if (has("import")) then (
        ...
        $IMPORT
      ) end | ([.import | to_entries[] | select(.value == null) | .key | "if is not var \"raw_import[" + . + "]\"; then raw_import[" + . + "]=\"$(json::from::yaml \".group |= walk(if type == \\\"object\\\" then with_entries(if .key == \\\"imported\\\" then .value |= \\\"$(normalizedpath \"$(dirname " + . + ")\")/\\\" + (. | sub(\\\"^[.]/\\\"; \\\"\\\")) else . end) else . end)\" " + . + ")\"; fi"] | join(";"))
    ')"
    import="$(json::encode raw_import)"

    raw_visited["${filepath}"]='true'
    visited="$(json::encode raw_visited)"
    filepath="$(normalizedpath "$(json::program --slurpfile IMPORT <(print '{"import": %s}' "${import}") --slurpfile VISITED <(print '%s' "${visited}") '
  ...
  json::filter "${jq[yml2bash/common]}${jq[yml2bash/write_script]}" --arg NAMESPACE_SEP "${sep[namespace]}" --arg EXE "${exe}" --arg BACKEND "${backend}" --rawfile FUNCTIONS <(declare -f "${fns[@]}") --args -- "${rainbow[@]}" \
    <<< "$(json::from::yaml --arg ROOT "$(normalizedpath "$(dirname "${1}")")/" --slurpfile INV <(print '%s' "${inv}") --slurpfile IMPORT <(print '{"import": %s}' "${import}") '
            (if ($IMPORT | type == "array") then $IMPORT[0] else $IMPORT end) as $IMPORT |
            (if ($INV | type == "array") then $INV[0] else $INV end) as $INV |
