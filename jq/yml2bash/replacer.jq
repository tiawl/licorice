  ...
  def replace_rec(walk_path; root_inventory): (
    if (type == "object") then (
      if (has("inventory")) then (
        ...
        map_values(replace_rec(walk_path; root_inventory))
      ) end
    ) elif (type == "array") then (
      map(replace_rec(walk_path; root_inventory))
    ) else . end
  );

  (.inventory as $root_inventory | $root_inventory | map_values(replace_rec([]; $root_inventory))) as $processed_inventory |
  {
    name: .name,
